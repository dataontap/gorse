...
class OXIOService:
    ...
    def create_subscription(self, line_id: str) -> Dict[str, Any]:
        """
        Create a subscription for Basic Membership with OXIO

        Args:
            line_id: Line ID from successful line activation
        ...
        try:
            ...
            # Get the Basic Membership plan ID from environment
            basic_membership_plan_id = os.environ.get('OXIO_BASIC_MEMBERSHIP_BASEPLANID')
            if not basic_membership_plan_id:
                return {
                    'success': False,
                    'error': 'Missing plan ID',
                    'message': 'OXIO_BASIC_MEMBERSHIP_BASEPLANID not configured in secrets'
                }

            payload = {
                "plan": {
                    "planId": basic_membership_plan_id
                },
                "lineId": line_id
            }

            print(f"OXIO Create Subscription URL: {url}")
            print(f"OXIO Create Subscription Headers (Auth masked): {dict(headers, **{'Authorization': '***'})}")
            print(f"OXIO Create Subscription Payload: {json.dumps(payload, indent=2)}")
            ...
            print(f"OXIO Create Subscription Response Status: {response.status_code}")
            print(f"OXIO Create Subscription Response Headers: {dict(response.headers)}")
            ...
            if 'application/json' in content_type:
                try:
                    response_data = response.json() if response.content else {}
                    print(f"OXIO Create Subscription Response Body (parsed): {json.dumps(response_data, indent=2)}")
                except json.JSONDecodeError as json_err:
                    print(f"Failed to parse JSON response: {str(json_err)}")
                    response_data = {
                        'error': 'Invalid JSON response',
            ...
            if response.status_code >= 200 and response.status_code < 300:
                return {
                    ...
                    'status_code': response.status_code,
                    'data': response_data,
                    'message': 'OXIO subscription created successfully',
                    'line_id': line_id,
                    'plan_id': basic_membership_plan_id,
                    'request_payload': payload
                }
            else:
                error_details = {
