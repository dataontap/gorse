...
@app.route('/api/send-datashare-invitation', methods=['POST'])
def send_datashare_invitation():
    """Send datashare group invitation"""
    try:
        data = request.get_json()
        if not data:
        ...
        firebase_uid = data.get('firebaseUid')
        group_name = data.get('groupName', 'My Datashare Group')
        invited_email = data.get('email')
        invited_name = data.get('name', 'Friend')
        ...
        group_owner_id = user_data[0]
        group_owner_email = user_data[1]
        group_owner_name = user_data[2] if len(user_data) > 2 else 'Group Owner'

        # For demo users, generate random email and name
        if is_demo_user:
            import time
            timestamp = int(time.time())
            invited_email = f"demo{timestamp}@datashare.example.com"
            invited_name = f"Demo User {timestamp % 1000}"

        if not invited_email:
            return jsonify({'success': False, 'message': 'Email is required for real invitations'}), 400
        ...
        with get_db_connection() as conn:
            if conn:
                with conn.cursor() as cur:
                    # Create datashare_invitations table if it doesn't exist
                    cur.execute("""
                    ...
                    # Check if this email was already invited by this owner
                    cur.execute("""
                        SELECT id, invitation_status FROM datashare_invitations 
                        WHERE group_owner_firebase_uid = %s AND invited_email = %s
                        ORDER BY created_at DESC LIMIT 1
                    """, (firebase_uid, invited_email))

                    existing_invite = cur.fetchone()
                    
                    if existing_invite and existing_invite[1] == 'invite_sent':
                        return jsonify({
                            'success': False, 
                            'message': f'{invited_email} has already been invited to {group_name}'
                        }), 400

                    # Insert new invitation
                    cur.execute("""
                        INSERT INTO datashare_invitations 
                        (group_owner_id, group_owner_firebase_uid, group_name, invited_email, 
                         invited_name, invitation_status, is_demo_user, personal_message, invitation_token)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                        RETURNING id
                    """, (group_owner_id, firebase_uid, group_name, invited_email, invited_name, 
                          'invite_sent', is_demo_user, personal_message, invitation_token))

                    invitation_id = cur.fetchone()[0]
                    conn.commit()

                    # Send email (simulate for demo users)
                    if is_demo_user:
                        print(f"=== DEMO DATASHARE INVITATION ===")
                        print(f"Demo invitation created for {invited_email}")
                        print(f"Group: {group_name}")
                        print(f"Owner: {group_owner_name} ({group_owner_email})")
                        print(f"Message: {personal_message}")
                        print("================================")
                    else:
                        # Send real email invitation
                        invitation_link = f"https://your-domain.com/join-datashare?token={invitation_token}"
                        
                        email_subject = f"You're invited to join {group_name} datashare group!"
                        email_body = f"""
Hello {invited_name},

{group_owner_name} has invited you to join their datashare group "{group_name}".

{f'Personal message: {personal_message}' if personal_message else ''}

Join this datashare group to:
• Share data allowances with group members
• Reduce connectivity costs together
• Manage group usage and settings
...
Best regards,
The DOT Wireless Team
                        """

                        print(f"=== DATASHARE INVITATION EMAIL ===")
                        print(f"To: {invited_email}")
                        print(f"Subject: {email_subject}")
...
@app.route('/api/datashare-invitations', methods=['GET'])
def get_datashare_invitations():
    """Get datashare invitations for a user"""
    try:
        firebase_uid = request.args.get('firebaseUid')
        if not firebase_uid:
            return jsonify({'success': False, 'message': 'Firebase UID required'}), 400
        ...
        with get_db_connection() as conn:
            if conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute("""
                        SELECT id, group_name, invited_email, invited_name, invitation_status,
                               is_demo_user, personal_message, created_at, updated_at,
                               accepted_at, rejected_at
                        FROM datashare_invitations 
                        WHERE group_owner_firebase_uid = %s
                        ORDER BY created_at DESC
                    """, (firebase_uid,))
                    ...
                    for invite in invitations:
                        invitation_list.append({
                            'id': invite[0],
                            'group_name': invite[1],
                            'invited_email': invite[2],
                            'invited_name': invite[3],
                            'invitation_status': invite[4],
                            'is_demo_user': invite[5],
                            'personal_message': invite[6],
                            'created_at': invite[7].isoformat() if invite[7] else None,
                            'updated_at': invite[8].isoformat() if invite[8] else None,
...
@app.route('/api/cancel-datashare-invitation/<int:invitation_id>', methods=['POST'])
def cancel_datashare_invitation(invitation_id):
    """Cancel a datashare invitation"""
    try:
        data = request.get_json() or {}
        firebase_uid = data.get('firebaseUid')
        ...
        with get_db_connection() as conn:
            if conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute("""
                        UPDATE datashare_invitations 
                        SET invitation_status = 'invite_cancelled', updated_at = CURRENT_TIMESTAMP
                        WHERE id = %s AND group_owner_firebase_uid = %s AND invitation_status = 'invite_sent'
                        RETURNING invited_email, group_name
                    """, (invitation_id, firebase_uid))

                    result = cur.fetchone()
                    if result:
                        conn.commit()
                        invited_email, group_name = result
                        return jsonify({
                            'success': True,
                            'message': f'Invitation to {invited_email} cancelled successfully'
                        })
                    else:
                        return jsonify({
                            'success': False,
        ...
        print(f"Error cancelling datashare invitation: {str(e)}")
        return jsonify({'success': False, 'message': str(e)}), 500
