...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initializeBlackHoleEffect();
});

function initializeBlackHoleEffect() {
    const dotIndicator = document.querySelector('.dot-indicator');
    if (!dotIndicator) return;
    
    // Create black hole animation elements
    createBlackHoleAnimation(dotIndicator);
    
    let pressTimer;
    let pressStarted = false;
    const pressTime = 180000; // 3 minutes in milliseconds
    let progressValue = 0;
    ...
    progressCircle.style.cssText = `
        ...
        background: conic-gradient(
            rgba(0, 102, 255, 0.5) ${progressValue}%, 
            transparent ${progressValue}%
        );
        opacity: 0;
        transition: opacity 0.3s ease;
    `;
    dotIndicator.appendChild(progressCircle);
    
    // Long press event handlers
    dotIndicator.addEventListener('mousedown', startPress);
    dotIndicator.addEventListener('touchstart', startPress, { passive: true });
    ...
    function startPress(e) {
        ...
        pressTimer = setInterval(() => {
            ...
            progressValue = (elapsed / pressTime) * 100;
            ...
            // If 3 minutes have passed, activate black hole
            if (elapsed >= pressTime) {
                clearInterval(pressTimer);
                progressCircle.style.opacity = '0';
                
                // Activate black hole animation
                if (typeof dotIndicator.activateBlackHole === 'function') {
                    dotIndicator.activateBlackHole();
                }
            }
        }, 50);
    }
    ...
    function endPress() {
        ...
        // Reset progress
        setTimeout(() => {
            progressCircle.style.background = 'conic-gradient(rgba(0, 102, 255, 0.5) 0%, transparent 0%)';
        }, 300);
    }
}
...
function createBlackHoleAnimation(element) {
    if (!element) return;

    // Create black hole container and elements
    const blackHoleContainer = document.createElement('div');
    blackHoleContainer.className = 'black-hole-container';
    
    // Create rings
    const ring1 = document.createElement('div');
    ring1.className = 'black-hole-ring ring1';
    
    const ring2 = document.createElement('div');
    ring2.className = 'black-hole-ring ring2';
    
    const ring3 = document.createElement('div');
    ring3.className = 'black-hole-ring ring3';
    
    const center = document.createElement('div');
    center.className = 'black-hole-center';
    
    // Add elements to container
    blackHoleContainer.appendChild(ring1);
    blackHoleContainer.appendChild(ring2);
    blackHoleContainer.appendChild(ring3);
    blackHoleContainer.appendChild(center);
    
    // Add container to dot indicator
    element.appendChild(blackHoleContainer);
    
    // Create stars function
    function createStars() {
        if (!element.classList.contains('black-hole-active')) return;
        ...
        blackHoleContainer.appendChild(star);
    ...
    element.activateBlackHole = function() {
        element.classList.add('black-hole-active');
        starInterval = setInterval(createStars, 200);
    };
    
    element.deactivateBlackHole = function() {
        element.classList.remove('black-hole-active');
        clearInterval(starInterval);
        
        // Remove existing stars
        const stars = blackHoleContainer.querySelectorAll('.star');
